...
public class KillScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Math.Sqrt(Math.Abs(playerX - botX) * Math.Abs(playerX - botX) + Math.Abs(playerZ - botZ) * Math.Abs(playerZ - botZ)) < 10 && role == "seeker") canKill = true;
        else canKill = false;
        if (Input.GetKey(KeyCode.E) && canKill && bot != null)
        {
            Destroy(bot);
